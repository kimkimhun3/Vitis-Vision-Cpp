...
// Worker-based processing with OpenCV histogram equalization + queue-level probes.
//
// Build:
...
static gboolean status_tick(gpointer user_data) {
    ...
    const uint64_t enq      = d->ctr.enqueued_frames.load();
    ...
    const uint64_t enq_b      = d->ctr.enqueued_bytes.load();
    ...
    g_print(
        "\n=== DEBUG STATUS (every 2s) ===  (work_q len=%d, push_fail=%" G_GUINT64_FORMAT ", proc_err=%" G_GUINT64_FORMAT ")\n"
        ...
        "enqueued to worker:  %8" G_GUINT64_FORMAT " frames, %8.2f MB\n"
    ...
    if (apps_in != enq)      g_print("LOSS between appsink.sink and enqueue:    %" G_GUINT64_FORMAT "\n", apps_in - enq);
    if (enq != proc)         g_print("LOSS in worker (enqueue -> process):      %" G_GUINT64_FORMAT "\n", enq - proc);
...
int main(int argc, char *argv[]) {
    ...
    g_print("Encoder: %s, target-bitrate: %d kbps, workers: %d\n", use_h265 ? "H.265" : "H.264", bitrate_kbps, num_workers);

    CustomData d{};
    d.work_q = g_async_queue_new();
    d.num_workers = num_workers;

    // Capture pipeline (bump queue to 8 for smoothing)
    GError *err=NULL;
    ...
    d.appsink = gst_bin_get_by_name(GST_BIN(sink_pipe), "my_sink");

    // Streaming pipeline (match basic.cpp settings)
    gchar *src_str=NULL;
    if (use_h265) {
    ...
    g_print("Huiiiiiiiiiiiii (OpenCV histogram equalization, worker-decoupled). Press Ctrl+C to exit.\n");

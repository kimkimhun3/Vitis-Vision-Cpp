...
int main(int argc, char *argv[]) {
    ...
    // Parse only --codec and --bitrate (kbps). Everything else remains untouched.
    gboolean use_h265 = FALSE;
    int bitrate_kbps = 20000; // default = 20 Mbps (matches your original 20000)
    ...
    // 1. Camera pipeline (appsink) — unchanged
    ...
    if (use_h265) {
        // H.265 path with dynamic target-bitrate (kbps)
        ...
        // Original H.264 path with dynamic target-bitrate (kbps)
        src_pipeline_str = g_strdup_printf(
            "appsrc name=my_src is-live=true format=GST_FORMAT_TIME do-timestamp=true ! "
            "video/x-raw,format=NV12,width=1920,height=1080,framerate=60/1 ! "

...
static GOptionEntry entries[] = {
    ...
    {"in", 'i', 0, G_OPTION_ARG_STRING, &in},
    {"out", 'o', 0, G_OPTION_ARG_STRING, &out},
    {"bitrate", 'b', 0, G_OPTION_ARG_INT, &bitrate},
    {"fps", 'f', 0, G_OPTION_ARG_STRING, &fps},
    {"width", 'w', 0, G_OPTION_ARG_INT, &v_width},
    {"height", 'h', 0, G_OPTION_ARG_INT, &v_height},
    {"k", 'k', 0, G_OPTION_ARG_INT, &k},
    {NULL}
  };
...
static GstFlowReturn new_sample_cb(GstAppSink *appsink, gpointer user_data) {
  ...
  if (GST_VIDEO_INFO_FORMAT(&data->video_info) == GST_VIDEO_FORMAT_YUY2) {
    cv::Mat yuy2_image_input = cv::Mat(data->video_info.height, data->video_info.width, CV_8UC2, map_info.data);
    cv::cvtColor(yuy2_image_input, converted_frame, cv::COLOR_YUV2BGR_YUY2);
  } else if (GST_VIDEO_INFO_FORMAT(&data->video_info) == GST_VIDEO_FORMAT_NV12) {
    cv::Mat nv12_image_input = cv::Mat(data->video_info.height + data->video_info.height / 2, data->video_info.width, CV_8UC1, map_info.data);
    cv::cvtColor(nv12_image_input, converted_frame, cv::COLOR_YUV2BGR_NV12);
  } else if (GST_VIDEO_INFO_FORMAT(&data->video_info) == GST_VIDEO_FORMAT_I420) {
    cv::Mat i420_image_input = cv::Mat(data->video_info.height + data->video_info.height / 2, data->video_info.width, CV_8UC1, map_info.data);
    ...
    converted_frame = cv::Mat(data->video_info.height, data->video_info.width, CV_8UC3, map_info.data, data->video_info.stride[0]);
  } else {
    g_print(
  ...
  try {
    ...
    // ------------------- MODIFICATION START -------------------
    // Calculate buffer sizes using the same macros as the kernel to guarantee a match.
    size_t input_buffer_size_bytes = height * width * XF_CHANNELS(IN_TYPE, NPPCX) * (XF_DTPIXELDEPTH(IN_TYPE, NPPCX) / 8);
    size_t output_buffer_size_bytes = height * width * XF_CHANNELS(OUT_TYPE, NPPCX) * (XF_DTPIXELDEPTH(OUT_TYPE, NPPCX) / 8);

    cl::Buffer imageToDevice1(data->context, CL_MEM_READ_ONLY, input_buffer_size_bytes);
    cl::Buffer imageToDevice2(data->context, CL_MEM_READ_ONLY, input_buffer_size_bytes);
    cl::Buffer imageFromDevice(data->context, CL_MEM_WRITE_ONLY, output_buffer_size_bytes);
    // -------------------- MODIFICATION END --------------------
    ...
    data->q.enqueueReadBuffer(imageFromDevice, CL_TRUE, 0,
                              output_buffer_size_bytes, out_img.data);
    // -------------------- MODIFICATION END --------------------
    ...
    g_warning("Unknown exception during processing.");
    ret = GST_FLOW_ERROR;
  }
#endif
  // ------------------------
...
int main(int argc, char *argv[]) {
  ...
  pipeline_str = g_strdup_printf(
      ...
      "rtph264pay mtu=1400 ! "
      "queue ! "
      "udpsink clients=192.168.25.20:5004 auto-multicast=false max-bitrate=%u",
      v_width, v_height, fps, target_bitrate_kbps, max_bitrate_bps);

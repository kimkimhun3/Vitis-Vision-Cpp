// histequalize_host.c — Clock-paced appsrc @60fps with ring buffer and last-frame repeat.
// - No upstream drops (appsink drop=false; bigger max-buffers)
// - If ring empty, repeat last buffer (zero-copy) to fill the 60fps slot
// - Explicit H.264 High/4.2 and H.265 Main/4.2 levels
#include <gst/gst.h>
...
typedef struct {
    ...
    GstBuffer    *last_buf;       // last good buffer (ref'ed) for repeats
    ...
    // Pipeline clock pacing
    GstClock     *clock;          // ref'ed
    GstClockTime  base_time;      // base time of appsrc pipeline
...
// appsink callback: enqueue latest buffer (drop oldest in ring if full)
static GstFlowReturn new_sample_cb(GstAppSink *appsink, gpointer user_data) {
    ...
    if (!d->video_info_valid) {
        ...
        if (caps && gst_video_info_from_caps(&d->video_info, caps)) {
            ...
            g_print("Negotiated: %dx%d %s @ %d/%d\n",
                    ...
                    d->video_info.fps_n, d->video_info.fps_d);
        } else {
    ...
    // Ref and push newest into ring; if full, drop oldest (ring policy)
    gst_buffer_ref(buffer);
    while (g_async_queue_length(d->queue) >= d->ring_capacity) {
        GstBuffer *old = (GstBuffer*)g_async_queue_try_pop(d->queue);
        if (old) gst_buffer_unref(old); else break;
    }
    g_async_queue_push(d->queue, buffer);

    // Update last_buf (keep ref)
    if (d->last_buf) gst_buffer_unref(d->last_buf);
    d->last_buf = gst_buffer_ref(buffer);

    gst_sample_unref(sample);
    return GST_FLOW_OK;
}

// Pusher: wait on pipeline clock each frame slot, set exact PTS/DUR, push.
// If ring empty -> push a zero-copy copy of last_buf to maintain 60fps.
static gpointer pusher_main(gpointer user_data) {
    ...
    while (G_LIKELY(d->running && !g_stop)) {
        // Compute target absolute clock time for this frame slot
        GstClockTime rt = gst_util_uint64_scale_int(++pushed, GST_SECOND, (guint)d->target_fps);
        GstClockTime target_abs = d->base_time + rt;

        // Wait on clock to align running-time
        GstClockID id = gst_clock_new_single_shot_id(d->clock, target_abs);
        (void)gst_clock_id_wait(id, NULL);
        gst_clock_id_unref(id);

        // Pop newest; if none, repeat last_buf (if available)
        GstBuffer *buf = (GstBuffer*)g_async_queue_try_pop(d->queue);
        if (!buf && d->last_buf) {
            buf = gst_buffer_copy(d->last_buf); // zero-copy duplicate of memory/meta
        }
        if (!buf) {
            // Nothing to push this slot
            continue;
        }

        // Stamp exact PTS/DURATION at target fps; let DTS be NONE
        GST_BUFFER_PTS(buf) = rt;
...
int main(int argc, char *argv[]) {
    ...
    d.ring_capacity  = 48;                 // ~0.8s cushion at 60fps
    d.target_fps     = fps;
    d.frame_duration = gst_util_uint64_scale_int(GST_SECOND, 1, (guint)fps);
    d.running        = TRUE;
    d.is_h265        = use_h265;

    // ---- Build appsink (capture) pipeline ----
    // IMPORTANT: avoid upstream drops; let us manage cadence in the pusher.
    gchar *sink_pipeline_str = g_strdup_printf(
        "v4l2src device=%s io-mode=dmabuf do-timestamp=true ! "
        "video/x-raw, format=NV12, width=%d, height=%d, framerate=%d/1 ! "
        // no leaky queue here; keep the path lossless into appsink
        "appsink name=cv_sink emit-signals=true max-buffers=60 drop=false sync=false",
    ...
    if (!use_h265) {
        ...
        src_pipeline_str = g_strdup_printf(
            ...
            "video/x-h265, alignment=au, profile=main, level=(string)4.2, stream-format=byte-stream ! "
    ...
    // appsrc is live; we set timestamps ourselves
    g_object_set(d.appsrc,
                 "is-live", TRUE,
                 "do-timestamp", FALSE,   // we stamp PTS/DUR explicitly
                 "block", TRUE,
                 "max-bytes", 6*1024*1024,
                 NULL);
    ...
    // Share same system clock across both pipelines
